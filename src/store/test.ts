...
export const test = {
  ...
  mutations: {
    ...
    setSubjectsPoints(state, questions) {
      ...
      const questionsArray = subjects.map((sub) =>
  ...
  getters: {
    ...
    getCategoriesResults: (state, getters) => {
      const processedQuestions = getters.processData
      const sections = processedQuestions
        .reduce((prev, curr) => {
      ...
      const scores = sections.map((section) => {
        const totalQuestionsInSection = processedQuestions.filter(
          (qa) => qa.subject == section
        )
        const gotCorrectInSection = totalQuestionsInSection.filter(
          (qa) => qa.gotCorrect
        )
        const gotIncorrectInSection = totalQuestionsInSection.filter(
          (qa) => !qa.gotCorrect
        )
        const answeringTimes = totalQuestionsInSection.map(
          (proQa) => proQa.answeredIn
        )
        const totalAnsweringTime = answeringTimes.reduce(
          (total, curr) => total + curr
        )
        ...
        const totalSectionPotentialScore = state.subjects
          .filter((sub) => sub.subject == section)
          .reduce((total, curr) => total + curr.points, 0)
        const totalSectionScores = gotCorrectInSection
          .map((qa) => qa.point)
          .reduce((total, curr) => total + curr, 0)
        ...
        return {
          ...
          scorePercentage:
            (totalSectionScores / totalSectionPotentialScore) * 100
        }
      })
      return scores
    },
    getAccuracyOverTime: (state, getters) => {
      const processedQuestions = getters.processData
      const questionsWithTime = processedQuestions.map((pq) => ({
        correct: pq.gotCorrect,
        time: pq.startAt
      }))
      ...
      const data = times.map(({ time }) => {
        ...
        const timeElapsedInMinutes = convertTimeToMinutes(
          time - state.testStartedAt
        )
    ...
    getSpeedOverTime: (state, getters) => {
      const processedQuestions = getters.processData
      return processedQuestions.map((pq) => ({
        answeredIn: pq.answeredIn,
        time: convertTimeToMinutes(pq.startAt - state.testStartedAt)
      }))
    }
  }
}
